...
const Table = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      axios
        .get("http://localhost/sipeta-ktp/backend/backbanyumanik/get_data.php")
        .then((response) => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write("<h2>Banyumanik</h2>");
    ...
    <div className="container">
      ...
      <div className="toolbar separated-toolbar">
        ...
        <div className="toolbar-right">
          <button className="btn-add" onClick={() => navigate("/add-data/banyumanik")}>
            Add New
          </button>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                <tr key={item.id}>
                  ...
                  <td>{item.tanggal}</td>
                  <td>
                    <button className="btn-view" onClick={() => navigate(`/view/banyumanik/${item.id}`)}>View</button>
                    <button className="btn-delete" onClick={() => navigate(`/delete/banyumanik/${item.id}`)}>Delete</button>
                  </td>
                </tr>
              ))
            ) : (
