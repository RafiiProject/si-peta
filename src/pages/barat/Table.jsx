...
const Table = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      axios
        .get("http://localhost/sipeta-ktp/backend/backbarat/get_data.php")
        .then((response) => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write("<h2>Semarang Barat</h2>");
  ...
  return (
    <div className="container">
      <div className="header-bar">
        <div>Semarang Barat</div>
        <div className="button-group">
          <button className="btn-add" onClick={() => navigate("/add-data/barat")}>Add New</button>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                <tr key={item.id}>
                  ...
                  <td>{item.tanggal}</td>
                  <td>
                    <button className="btn-view" onClick={() => navigate(`/view/barat/${item.id}`)}>View</button>
                    <button className="btn-delete" onClick={() => navigate(`/delete/barat/${item.id}`)}>Delete</button>
                  </td>
                </tr>
              ))
            ) : (
